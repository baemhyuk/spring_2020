...
var replyService = (function() {
   ...
   function getList(param, callback, error) {
      ...
      var prog_num = param.prog_num;
      var page = param.page || 1; // param.page 가 null 이면 1로 설정 
      
      $.getJSON("/replies/pages/" + prog_num + "/" + page + ".json", function(data) {
         if (callback) {
   ...
   // 댓글 삭제. security 적용 후
   function remove(rno, user_id, callback, error) {
        
      console.log("--------------------------------------");  
      console.log(JSON.stringify({rno:rno, user_id:user_id}));  
          
      $.ajax({
         type : 'delete',
         url : '/replies/' + rno,
         data:  JSON.stringify({rno:rno, user_id:user_id}),
         contentType: "application/json; charset=utf-8",
